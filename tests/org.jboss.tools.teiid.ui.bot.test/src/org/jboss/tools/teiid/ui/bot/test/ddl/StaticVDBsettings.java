...
public class StaticVDBsettings {
	...
	private void checkVDB(){
		...
		if (new JiraClient().isIssueClosed("TEIIDDES-2990")){
			...
			collector.checkThat("Translator name for first model is bad set", sources.get(0).getText(1), is("sqlserver"));
			collector.checkThat("JNDI name for first model is bad set", sources.get(0).getText(2), is("sourceModel"));
			
			collector.checkThat("Source name for second model is bad set", sources.get(1).getText(0), is("second"));
			collector.checkThat("Translator name for second model is bad set", sources.get(1).getText(1), is("sqlserver"));
			collector.checkThat("JNDI name for second model is bad set", sources.get(1).getText(2), is("second"));
		}
